...
namespace cusort {
  ...
  template <typename Key_t,typename Value_t, typename Length_t,
            Length_t MAX_NUM_GPUS = 16, Length_t BIN_SCALE = 16,
            Length_t BLOCK_DIM = 128, Length_t MEM_ALIGN = 512>
  class Cusort {
  public:
    Cusort() {
    ...
    public:
      ...
      void allocate(Length_t len, Length_t cubData) {
        Length_t cubDataSize = ((cubData + MEM_ALIGN - 1L) / MEM_ALIGN) * MEM_ALIGN;
        Length_t sdSize = ((len + MEM_ALIGN - 1L) / MEM_ALIGN) * MEM_ALIGN;
    ...
    struct ThreadData {
      ...
      void allocate(int32_t num_bins, int num_gpus) {
        ...
        cudaMemset(binSizes, 0, (num_bins + 1) * sizeof(Key_t));
...
#pragma omp master
      ...
      cudaMemcpy(tData[cpu_tid].h_binPrefix, tData[cpu_tid].binPrefix, (num_bins+1)*sizeof(Length_t), cudaMemcpyDeviceToHost);
...
#pragma omp barrier
      ...
      cudaMemcpy(tData[cpu_tid].binMap, h_binMap, num_bins * sizeof(unsigned char), cudaMemcpyHostToDevice);
      cudaMemcpy(tData[cpu_tid].tempPrefix, h_readPositions[cpu_tid+1], (num_gpus + 1) * sizeof(Length_t), cudaMemcpyHostToDevice);
      ...
      if (elements > (1L << 31)) {
        printf("input column is too big");
      }
...
#pragma omp barrier
      ...
      for (int other = 0 ; other < num_gpus ; ++other) {
        ...
        cudaMemcpyAsync(tData[cpu_tid].d_output_keys + h_writePositionsTransposed[cpu_tid][from_id],
        ...
        if (!keys_only)
          cudaMemcpyAsync(tData[cpu_tid].d_output_values + h_writePositionsTransposed[cpu_tid][from_id],
...
#pragma omp barrier
      ...
      cudaMemcpy(tData[cpu_tid].d_output_keys, tData[cpu_tid].bdReorder.d_keys, tData[cpu_tid].h_output_length * sizeof(Key_t), cudaMemcpyDeviceToDevice);

      if (!keys_only)
        cudaMemcpy(tData[cpu_tid].d_output_values, tData[cpu_tid].bdReorder.d_vals, tData[cpu_tid].h_output_length * sizeof(Value_t), cudaMemcpyDeviceToDevice);
...
#pragma omp barrier
    ...
    void sort(Key_t **d_input_keys,
      ...
      if ((Length_t)num_gpus > MAX_NUM_GPUS) {
        printf("num_gpus > MAX_NUM_GPUS");
      }
...
#pragma omp barrier
    ...
    void sort(Key_t **d_input_keys,
      ...
      if (num_gpus > MAX_NUM_GPUS) {
        printf("num_gpus > MAX_NUM_GPUS in sort");
      }
